...
void apply_blur(unsigned char *my_frame, int frame_width, int frame_height)
{
    ...
    for (int i = 0; i < frame_width; i++)
    {
        for (int j = 0; j < frame_height; j++)
        {
            ...
            for (int filterX = i - RADIUS; filterX <= i + RADIUS; filterX++)
            {
                for (int filterY = j - RADIUS; filterY <= j + RADIUS; filterY++)
                {
                    if (filterX >= 0 && filterX < frame_width &&
                        filterY >= 0 && filterY < frame_height)
                    {
                        int nindex = (filterX * frame_height + filterY) * bytes_per_pixel;
            ...
            my_frame[idx]     = cv::saturate_cast<uchar>(sumB / 9);
            my_frame[idx + 1] = cv::saturate_cast<uchar>(sumG / 9);
            my_frame[idx + 2] = cv::saturate_cast<uchar>(sumR / 9);
        }
    }
